...
columns_to_encode <- c('Customer.Segment', 'Department.Name', 'Order.Region', 'Product.Name')
...
class(model1_LRM)
summary(model1_LRM) 
y_pred <- predict(model1_LRM, newdata = x_test)

# Model evaluation metrics
mae <- mean(abs(y_test - y_pred))
mse <- mean((y_test - y_pred)^2)
rmse <- sqrt(mse)
rsq <- cor(y_pred, y_test)^2
...
res_df <- nrow(x_train) - length(coef(model1_LRM))
...
set.seed(123)
index <- createDataPartition(y, times = 0.75, list = F) 
...
# Tune model
ridge_tuned <- train(x_train, y_train, method = "glmnet", tuneGrid=tune_grid)

best_lambda <- ridge_tuned$bestTune$lambda

# Refit final model
ridge_model <- glmnet(x_train, y_train, alpha = 0, lambda = best_lambda)

# Evaluate on test data
y_pred <- predict(ridge_model, newx = as.matrix(x_test))
...
library(rpart)
x_train_model <- model.matrix(y_train ~ ., data = x_train)  

# Test model matrix 
x_test_model <- model.matrix(y_train ~., data = x_test)

# Decision Tree Model
model3_DTR <- rpart(y_train ~ ., data = x_train_model,  
                    method = "anova",
...
pred <- predict(model, newx = x_test_model)
...
model4_RF <- randomForest(y_train ~ ., data = x_train, 
                          ntree = 100, 
                          maxdepth = 15, 
                          nodesize = 5, 
                          minsplit = 30,
...
# Making predictions on the test data
y_pred <- predict(model4_RF, newdata = x_test)
...
# Assuming 'Product' is the correct column name and 'Count' is the frequency of each product
ggplot(your_data_frame, aes(x = reorder(Product, Count), y = Count)) +
